import { tablaDeSimbolos } from "./analizadorLexico.js";

export const tablaDeFunciones = [];
export const tablaDeVariables = [];
export let errorEncontrado;
let pos, existe, posicion, argumentosDeFuncion;

export function mainSintactico() {
    errorEncontrado = "";
    ...
    if (tablaDeSimbolos.length > 0) {
        ...
        for (var i = 0; i < tablaDeSimbolos.length; i++) {
            //! Comprueba si existe class en la tabla de simbolos
            ...
            //! Comprueba si existe var en la tabla de simbolos
            ...
            //! Comprueba si se crean variables en la subclase var
            if (
            ...
            //! Comprueba si se crea main dentro de la clase principal
            ...
            if (tablaDeSimbolos[i].opCode == "mi") {
                const [evaluaMetodos, nuevai] = comprobarMetodo(
                    tablaDeSimbolos,
                    i
                );
            ...
            if (tablaDeSimbolos[i].opCode == "fcm") {
                if (
                    !cierreDeFuncion(tablaDeFunciones, tablaDeSimbolos[i].linea)
                ) {
                    errorEncontrado =
                        errorEncontrado +
                        "\n Error en la linea: " +
            ...
            if (tablaDeSimbolos[i].opCode == "fc") {
                if (
                    !cierreDeClase(tablaDeFunciones, tablaDeSimbolos[i].linea)
                ) {
                    errorEncontrado =
                        errorEncontrado +
                        "\n Error en la linea: " +
            ...
            if (tablaDeSimbolos[tablaDeSimbolos.length - 1].opCode != "fc") {
                errorEncontrado =
                    errorEncontrado +
                    "No existe cierre de la clase principal. ";
                errorSintactico(errorEncontrado);
            }
        ...
        if (exprecion[i] == "$") {
            ...
            if (variable[0]) {
                ...
                errorEncontrado =
                    ...
                    "\nLa variable '" +
                    nombre +
                    "' no existe. Declarala primero en 'var'";
                return [false, 0];
            }
        }
    }
            ...
            if (miPadreEs == "c") {
                if (soySuHijo) {
                    if (
                        ...
                        errorEncontrado =
                            errorEncontrado +
                            "No se puede declarar el function '" +
                            nombreVariable +
                            "', la clase '" +
                            miHermanoEs +
                            "' no esta cerrada. O el function no esta enseguida de la clase 'var'";
                        return false;
                    }
                } else {
                    errorEncontrado =
                        errorEncontrado +
                        "No se puede declarar un function fuera de la clase base.";
                    return false;
                }
            } else {
                errorEncontrado =
                    errorEncontrado +
                    "No se puede declarar un function sin la existencia de una clase base.";
                return false;
            }
            break;
        case 5:
            ...
            if (miPadreEs == "c") {
                if (soySuHijo) {
                    if (miHermanoEs == "iVar" && !miHermanoCerro) {
                        ...
                        errorEncontrado =
                            errorEncontrado +
                            "No se puede declarar la variable fuera de la clase var'" +
                            nombreVariable +
                            "', no se encuentra dentro de la clase 'var', o esta ya cerro.";
                        return false;
                    }
                } else {
                    errorEncontrado =
            ...
            if (OrigenOPcode[pos].opCode == "arg") {
                ...
                if (OrigenOPcode[pos].opCode == "rp:") {
                    ...
                    if (OrigenOPcode[pos].opCode == "rp:") {
                        ...
                        errorEncontrado =
                            "Falta el caracter ')' de cierre de la clase principal";
                        return [false, pos];
                    }
                } else {
                    errorEncontrado =
                        "Faltan el caracter '(' de apertura para argumentos";
                    return [false, pos];
                }
            } else {
                errorEncontrado = "Palabra reserva: 'args' no encontrada.";
                return [false, pos];
            }
        } else {
            errorEncontrado =
...
//!Metodo para comprobar la construccion de var de nuestro programa
    ...
    if (tengoLosPadresCorrectos(OrigenOPcode, i)) {
        ...
        if (OrigenOPcode[i].opCode == "iVar") {
            ...
            if (OrigenOPcode[i].opCode == "rp:") {
                i++;
                //Recuperar los argumentos de var
                const [evaluaArgumentos, argumentos, nuevai] =
                ...
                if (OrigenOPcode[i].opCode == "rp:") {
                    ...
                    errorEncontrado =
                        "Falta el caracter ')' de cierre de declaracion de variables";
                    return [false, i];
                }
            } else {
                errorEncontrado =
                    "Faltan el caracter ')' de apertura para declaracion de variables";
                return [false, i];
            }
        } else {
            errorEncontrado = "Palabra reserva: 'var' no encontrada.";
            return [false, i];
        }
    } else {
        errorEncontrado =
            errorEncontrado +
            "\nComprueba  los padres de la declaracion de var";
        return [false, i];
    }
}
        ...
        if (OrigenOPcode[pos].opCode == "cmi") {
            ...
            if (OrigenOPcode[pos].opCode == "rp:") {
                ...
                if (evaluaArgumentos == true) {
                    ...
                    errorEncontrado =
                        errorEncontrado +
                        "\n Error en los argumentos de main";
                    return [false, pos];
                }
                if (OrigenOPcode[pos].opCode == "rp:") {
                    pos++;
                    if (OrigenOPcode[pos].opCode == "io") {
                        ...
                        errorEncontrado =
                            "Falta el caracter '(' de apertura de main";
                        return [false, pos];
                    }
                } else {
                    errorEncontrado =
                        "Falta el caracter ')' de cierre de main";
                    return [false, pos];
                }
            } else {
                errorEncontrado =
                    "Faltan el caracter '(' de apertura para main";
                return [false, pos];
            }
        } else {
            errorEncontrado =
                "Palabra reserva: 'main' no encontrada.";
            return [false, pos];
        }
    } else {
        errorEncontrado = +errorEncontrado;
        ("No se encontro la clase principal. No puedes declarar main fuera de la clase principal.");
        return [false, pos];
    }
}

//!Metodo para comprobar la construccion de variables de nuestro programa dentro de la subclase var
function declaracionVariables(origenOPcode, pos) {
    if (tengoLosPadresCorrectos(origenOPcode, pos)) {
        ...
        if (
            origenOPcode[pos].opCode == "tdT" ||
            origenOPcode[pos].opCode == "tdP"
        )
            caso = 1;
        ...
        switch (caso) {
            case 1:
                ...
                if (origenOPcode[pos].opCode == "nVar") {
                    ...
                    errorEncontrado =
                        errorEncontrado +
                        "Se esperaba el nombre de variable despues de el tipo de dato 'int'";
                    return [false, pos];
                }
                break;
            case 2:
                ...
                if (origenOPcode[pos].opCode == "nVar") {
                    ...
                    errorEncontrado =
                        errorEncontrado +
                        "Error en la sentencia, se esperaba el nombre de variable despues de el tipo de dato 'string'";
                    return [false, pos];
                }
                break;
            case 3:
        ...
        errorEncontrado =
            errorEncontrado +
            "Parece que la variable esta donde no debe estar.";
        return [false, pos + 1];
    }
}
        ...
        if (OPcode[pos].opCode == "nCM") {
            ...
            if (OPcode[pos].opCode == "rp:") {
                ...
                if (evaluaArgumentos) {
                    ...
                    errorEncontrado =
                        errorEncontrado + "Error en los argumentos del function";
                    return [false, pos];
                }
                if (OPcode[pos].opCode == "rp:") {
                    pos++;
                    if (OPcode[pos].opCode == "ci") {
                        ...
                        if (OPcode[pos].opCode == "io") {
                            pos++;
                            let [sentencias, nuevai] =
                                construirTablaDeSentencias(OPcode, pos);
                            pos = nuevai;
                            ...
                            errorEncontrado =
                                "Se esperaba ':' despues de la sentencia '_init'";
                        }
                    } else {
                        errorEncontrado =
                            "Se esperaba '_init' despues de la sentencia ')'";
                    }
                } else {
                    errorEncontrado =
                        "Se esperaba ')' para el cierre de la declaracion de argumentos del function";
                }
            } else {
                errorEncontrado =
                    "Error en la sentencia, no se encontro '(' despues del nombre del function";
            }
        } else {
            errorEncontrado =
                errorEncontrado + "Se esperaba el nombre del function";
            return [false, pos];
        }
    } else {
        errorEncontrado =
            +errorEncontrado +
            "\nError en la sentencia, el function '" +
            OPcode[pos + 1].token +
        ...
        switch (opCode) {
            ...
            case "edcAl":
                ...
                if (sentencia[1].opCode == "io") {
                    ...
                    errorEncontrado =
                        "Se esperaba ':' despues de la sentencia 'else'";
                    return [false, i];
                }
                break;
    ...
    if (variable1[0]) {
        ...
        if (sentencia[i].opCode == "asig=") {
            ...
            if (sentencia[i].opCode == "cad") {
                ...
                const [evaluaExpresion, pos, evaluada] = aritLogic(
                    sentencia,
                    i
                );
                if (evaluaExpresion) {
                ...
                sentencia[i].opCode == "tdV" ||
                sentencia[i].opCode == "tdF"
            ) {
        ...
        errorEncontrado =
            ...
            sentencia[0].token +
            " no existe. Declarala antes en la clase 'var'";
        return false;
    }
}
    ...
    if (sentencia[pos].opCode == "rp:") {
        ...
        if (sentencia[pos].opCode == "nVar") {
            ...
            if (sentencia[pos].opCode == "rp:") {
                ...
                errorEncontrado =
                    errorEncontrado +
                    "Se esperaba un ')' despues de el nombre de la variable en la sentencia 'input' " +
                    "Error en la linea" +
                    sentencia[pos].linea;
                return false;
            }
            ...
            errorEncontrado =
                errorEncontrado +
                "Error en la funcion nativa 'input', se esperaba una variable en la que recibir el  input." +
                "Error en la linea" +
                sentencia[pos].linea;
            return false;
        }
        ...
        errorEncontrado =
            errorEncontrado +
            "Error en la sentencia de input 'input', se esperaba '(' antes del nombre de la variable" +
            "Error en la linea" +
            sentencia[pos].linea;
        return false;
    }
    ...
    if (sentencia[pos].opCode == "rp:") {
        ...
        if (
            ...
            if (sentencia[pos].opCode == "mfCh") {
                pos++;
                let llamadaCorrecta = sentenciaLlamadaFuncionChimare(
                    sentencia,
                    pos
                );
            ...
            if (sentencia[pos].opCode == "rp:") {
                ...
                errorEncontrado =
                    errorEncontrado +
                    "Se esperaba un ')' despues de el nombre de la variable" +
                    "Error en la linea " +
                    sentencia[pos].linea;
                return false;
            }
        ...
        errorEncontrado =
            errorEncontrado +
            "Error en la sentencia de output (print), se esperaba '(' antes del nombre de la variable" +
            "Error en la linea " +
            sentencia[pos].linea;
        return false;
    }
...
function sentenciaLlamadaFuncionChimare(sentencia, pos) {
    if (sentencia[pos].opCode == "rp:") {
        ...
        if (sentencia[pos].opCode == "nCM") {
            ...
            if (sentencia[pos].opCode == "rp:") {
                ...
                errorEncontrado =
                    ...
                    nombreFuncion +
                    " se esperaba '(' despues del nombre de la funcion para la declaracion de los parametros";
                sentencia[pos].linea;
                return false;
            }
        } else {
            errorEncontrado =
                errorEncontrado +
                "Se esperaba el nombre de la funcion a llamar en la funcion call." +
                "Error en la linea " +
                sentencia[pos].linea;
            return false;
        }
        ...
        errorEncontrado =
            errorEncontrado +
            "Error en la sentencia de llamada de funcion, se esperaba el operador '(' para el inicio de los argumentos de call." +
            "Error en la linea " +
            sentencia[pos].linea;
        return false;
    }
...
function sentenciaCicloFor(sentencia, pos) {
    if (sentencia[pos].opCode == "rp:") {
        ...
        if (sentencia[pos].opCode == "nVar") {
                ...
                if (sentencia[pos].opCode == "numE") {
                    ...
                    if (sentencia[pos].opCode == "sep,") {
                        ...
                        if (sentencia[pos].opCode == "or<") {
                            ...
                            if (sentencia[pos].opCode == "nVar") {
                                ...
                                if (sentencia[pos].opCode == "rp:") {
                                    ...
                                    if (sentencia[pos].opCode == "io") {
                                        ...
                                        errorEncontrado =
                                            errorEncontrado +
                                            "Se esperaba el operador de inicio ':' en la linea " +
                                            sentencia[pos].linea;
                                        return false;
                                    }
                                } else {
                                    errorEncontrado =
                                        errorEncontrado +
                                        "Error en declaracion for, se esperaba un operador '(' . Error en la linea " +
                                        sentencia[pos].linea;
                                    return false;
                                }
                            } else {
                                errorEncontrado =
                                    errorEncontrado +
                                    "Se esperaba una variable despues del operador condicional en la sentencia de ciclo for. Error en la linea " +
                                    sentencia[pos].linea;
                                return false;
                            }
                        } else {
                            errorEncontrado =
                                errorEncontrado +
                                "Error en la sentencia de ciclo for, se esperaba condicional despues de la coma. " +
                                "Error en la linea " +
                                sentencia[pos].linea;
                            return false;
                        }
                    ...
                    errorEncontrado =
                        ...
                        "Error en la linea " +
                        sentencia[pos].linea +
                        "Se esperaba un numero entero desde el que inciar el ciclo for";
                    return false;
                }
            } else {
                errorEncontrado =
            ...
            errorEncontrado =
                errorEncontrado +
                "Se esperaba una variable de indice para el ciclo for. Error en linea " +
                sentencia[pos].linea;
            return false;
        }
    } else {
        errorEncontrado =
            errorEncontrado +
            "Se esperaba el uso del operador '(' despues de la palabra reservada for. Error en la linea " +
            sentencia[pos].linea;
        return false;
    }
}
...
function sentenciaCondicionIf(sentencia, pos) {
    if (sentencia[pos].opCode == "rp:") {
        ...
        errorEncontrado =
            errorEncontrado +
            "Se esperaba el operador '(' despues de la palabra reservada 'if'. Error en la linea " +
            sentencia[pos].linea;
    }
}
...
function aritLogic(OrigenOPcode, i) {
    ...
    while (exprecion.includes("$")) {
        ...
        const [evaluacionV, exprecionNueva] =
            resolverValoresDeVariables(exprecion);
        if (evaluacionV) {
